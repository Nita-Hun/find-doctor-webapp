...
export default function SpecializationsPage() {
  ...
  const fetchSpecializations = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get('http://localhost:8080/api/specializations');
      console.log('API Response:', response.data);
      
      const specializationsData = Array.isArray(response.data) 
        ? response.data 
        : response.data?.content || response.data?.specializations || [];
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Are you sure you want to delete this specialization?')) {
      try {
        await axios.delete(`http://localhost:8080/api/specializations/${id}`);
  ...
  const filteredSpecializations = specializations.filter(spec => {
    ...
    return specName.toLowerCase().includes(searchTerm.toLowerCase());
  });
                ...
                {paginatedSpecializations.map((spec) => (
                  <tr key={spec.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-500">{spec.description || 'No description'}</div>
                    </td>
                ...
                <div>
                  <p className="text-sm text-gray-700">
                    ...
                    <span className="font-medium">
                      {Math.min(currentPage * pageSize, filteredSpecializations.length)}
                    </span>{' '}
                    of <span className="font-medium">{filteredSpecializations.length}</span> results
                  </p>
                </div>
                <div>
